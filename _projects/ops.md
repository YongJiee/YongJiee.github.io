...
|------|---------|----------------|
| `multi_camera_publisher` | Pi | Captures 3x IMX708 via Arducam mux, publishes `/image_raw` |
...
- **Cam0 + Cam2** at 45 degrees - side faces
...
def _calculate_accuracy(self, ocr_tokens, candidate):
    ...
    if self._is_unique_product(candidate):
        return product_score * 0.9 + keyword_bonus
    elif brand_score == 0:
        return brand_score * 0.6
    else:
...
The Arducam multiplexer switches cameras via I2C - it physically cannot expose more than one sensor at a time. `ThreadPoolExecutor` for parallel capture was tested and ruled out. Instead, I squeezed latency out of each capture: focus delay reduced from 0.2s to 0.05s, local disk save removed, and redundant RGB to BGR conversion eliminated. Net result: Pi cycle time of 2.1-2.5s for 3 cameras.
...
|------------|-----------|---------|
| Inbound scanning | 10 | All 3.0s or under |
| Sorting mode | 8 | All 2.7s or under |
...
| Cameras | 3x IMX708, Arducam Multi Camera Adapter V2.2 |
